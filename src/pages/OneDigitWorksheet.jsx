// src/pages/OneDigitWorksheet.jsx
...
const OneDigitWorksheet = () => {
  ...
  const formulaOptions = [
    { value: 'none', label: '🔹 No Formula (Direct Only)' },
    { value: '+1', label: '➕ +1 Small Friend (+5-4)' },
    { value: '+2', label: '➕ +2 Small Friend (+5-3)' },
    { value: '+3', label: '➕ +3 Small Friend (+5-2)' },
    { value: '+4', label: '➕ +4 Small Friend (+5-1)' },
    { value: '-1', label: '➖ -1 Small Friend (-5+4)' },
    { value: '-2', label: '➖ -2 Small Friend (-5+3)' },
    { value: '-3', label: '➖ -3 Small Friend (-5+2)' },
    { value: '-4', label: '➖ -4 Small Friend (-5+1)' },
    { value: '+6', label: '🧮 +6 Big Friend (+10-4)' },
    { value: '+7', label: '🧮 +7 Big Friend (+10-3)' },
    { value: '+8', label: '🧮 +8 Big Friend (+10-2)' },
    { value: '+9', label: '🧮 +9 Big Friend (+10-1)' },
    { value: '-6', label: '🧮 -6 Big Friend (-10+4)' },
    { value: '-7', label: '🧮 -7 Big Friend (-10+3)' },
    { value: '-8', label: '🧮 -8 Big Friend (-10+2)' },
    { value: '-9', label: '🧮 -9 Big Friend (-10+1)' },
  ...
  const generateNewWorksheet = async () => {
    setGenerating(true);
    const formulaValue = formula === 'none' ? null : formula;
    const newProblems = generateWorksheet(formulaValue);
    ...
    try {
      ...
      const { data: worksheetData, error: worksheetError } = await supabase
        ...
        .insert({
          ...
          questions: { problems, formula: formula === 'none' ? null : formula },
        })
        .select()
        .single();
      ...
      // Update user profile stats (optional)
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-6">
      <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
        <h1 className="text-3xl font-black text-[#1A1A1A] mb-2">🧮 1‑Digit Abacus Sums</h1>
        <p className="text-[#555555]">Practice small & big friends · 6‑number chains</p>
...
export default OneDigitWorksheet;